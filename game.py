...
def _ensure_db_dir(path: str) -> None:
    """Create parent directory for the SQLite file if needed."""
...
def init_db() -> None:
    """Create the players table. Falls back to ./ghost_hunter.db if the
    configured path is not writable (common when a volume mount is missing).
    """
    global DB_PATH
    try:
        _ensure_db_dir(DB_PATH)
        ...
        # e.g. GHOST_HUNTER_DB=/data/ghost_hunter.db but no volume at /data
        fallback = "ghost_hunter.db"
        print(
            f"[ghost_hunter] WARNING: cannot open DB at {DB_PATH!r}; "
            f"falling back to {fallback!r}. "
            "If you set GHOST_HUNTER_DB, mount a Railway volume at that path "
